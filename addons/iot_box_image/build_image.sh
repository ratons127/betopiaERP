...
# Generate a unique self-signed TLS cert for nginx inside the image.
# Do not ship a repo-wide private key in overwrite_after_init.
mkdir -pv "${MOUNT_POINT}/etc/ssl/private" "${MOUNT_POINT}/etc/ssl/certs"
openssl req -x509 -newkey rsa:2048 -nodes -days 3650 \
    -subj "/CN=BetopiaERPTempIoTBoxCertificate" \
    -keyout "${MOUNT_POINT}/etc/ssl/private/nginx-cert.key" \
    -out "${MOUNT_POINT}/etc/ssl/certs/nginx-cert.crt"
chmod 600 "${MOUNT_POINT}/etc/ssl/private/nginx-cert.key"
