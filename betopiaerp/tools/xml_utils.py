...
def load_xsd_files_from_url(env, url, file_name=None, force_reload=False,
    ...
    from .net import fetch_url_content  # noqa: PLC0415
    try:
        ...
        _logger.warning('Invalid URL: %s (%s)', url, error)
        return False
    ...
    if total_uncompressed > 100 * 1024 * 1024:
        _logger.warning("ZIP archive too large from %s", url)
        return False
    for file_path in archive.namelist():
        ...
        if len(content) > 10 * 1024 * 1024:
            _logger.warning("Skipping oversized XSD file %s from ZIP archive", file_name)
            continue
        if modify_xsd_content:
            content = modify_xsd_content(content)
